...
export async function getUserAiConfig(
  ...
  const res = await pool.query<Record<string, unknown>>(
    `SELECT ${PROVIDER_COLUMNS.join(', ')}, provider, agent_settings
     FROM bot_user_ai_config WHERE user_id = $1`,
    [userId],
  );
...
export async function saveUserAgentSettings(
  ...
  const current = await pool.query<{ agent_settings: string | null }>(
    `SELECT agent_settings FROM bot_user_ai_config WHERE user_id = $1`,
    [userId],
  );
  const merged = {
  ...
  await pool.query(
    `INSERT INTO bot_user_ai_config
     ...
     ON CONFLICT (user_id) DO UPDATE SET
       ...
       updated_at  = NOW()`,
    [userId, encryptedKey, keyHint, model, provider],
  );
}
...
export async function updateUserAiModel(
  ...
  const modelCol = providerColumn(provider, 'model');
  await pool.query(
    `UPDATE bot_user_ai_config
     SET provider = $2, ${modelCol} = $3, updated_at = NOW()
     WHERE user_id = $1`,
...
export async function deleteUserAiKey(
  ...
  await pool.query(
    `UPDATE bot_user_ai_config
     SET ${keyCol} = '', ${hintCol} = '', updated_at = NOW()
     WHERE user_id = $1`,
    [userId],
  );
}
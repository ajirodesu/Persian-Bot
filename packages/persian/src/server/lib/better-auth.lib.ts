...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    ...(isEmailServicesEnabled && {
      sendResetPassword: async ({ user, url }) => {
        ...
        void sendMail({
          to: user.email,
          subject: 'Reset your Persian password',
          html: buildEmailLayout(
            `
            <p style="margin: 0 0 16px 0; color: ${COLORS.onSurface}; font-weight: 500;">Hello ${String(user.name ?? user.email)},</p>
            <p style="margin: 0 0 24px 0;">Click the button below to reset your Persian password:</p>
          ...
          text: `Reset your Persian password by visiting: ${finalUrl}`,
        });
      },
    }),
  },
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user, url }) => {
      ...
      void sendMail({
        to: user.email,
        subject: 'Verify your Persian email address',
        html: buildEmailLayout(
          `
          <p style="margin: 0 0 16px 0; color: ${COLORS.onSurface}; font-weight: 500;">Hello ${String(user.name ?? user.email)},</p>
          <p style="margin: 0 0 24px 0;">Click the button below to verify your email and activate your Persian account:</p>
          ${buildButton(url, 'Verify Email')}
          <p style="margin: 24px 0 0 0; color: ${COLORS.outlineVariant}; font-size: 14px;">This link expires in 1 hour. If you did not sign up for Persian, you can safely ignore this email.</p>
        `,
          'Verify your email address',
        ),
        text: `Verify your Persian email address by visiting: ${url}`,
      });
    },
  },
  ...
  rateLimit: {
    ...
    // process restart, which is acceptable for Persian's single-process deployment.
    enabled: true,
    window: 60,
    max: 100,
    storage: 'memory',
  ...
  emailVerification: {
    ...(isEmailServicesEnabled && {
      sendVerificationEmail: async ({ user, url }) => {
        void sendMail({
          to: user.email,
          subject: 'Verify your Persian Admin email address',
          html: buildEmailLayout(
            `
            <p style="margin: 0 0 16px 0; color: ${COLORS.onSurface}; font-weight: 500;">Hello ${String(user.name ?? user.email)},</p>
          ...
          text: `Verify your Persian Admin email address by visiting: ${url}`,
        });
      },
    }),
  },
  emailAndPassword: {
    ...
    ...(isEmailServicesEnabled && {
      sendResetPassword: async ({ user, url }) => {
        ...
        void sendMail({
          to: user.email,
          subject: 'Reset your Persian Admin password',
          html: buildEmailLayout(
            `
            <p style="margin: 0 0 16px 0; color: ${COLORS.onSurface}; font-weight: 500;">Hello ${String(user.name ?? user.email)},</p>
          ...
          text: `Reset your Persian Admin password by visiting: ${finalUrl}`,
        });
      },
    }),
  },

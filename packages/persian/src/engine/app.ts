/**
 * Persian — Central Entry Point & Orchestration Layer
 *
...
async function main(): Promise<void> {
  logger.info('Persian - loading modules...');
  ...
  logger.info('Persian - creating platform listeners...');
  ...
  logger.info('Persian - starting all platforms...');
  ...
  logger.info('Persian — all platform listeners wired');
...
main().catch((err: unknown) => {
  logger.error('💀 Fatal: could not start Persian', { error: err });
  process.exit(1);
});

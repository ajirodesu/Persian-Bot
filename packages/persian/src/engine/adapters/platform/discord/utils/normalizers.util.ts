 ...
 * unified Persian event contract (PROTO_EVENT_* shapes from models/prototypes/).
 *
...
/**
 * Normalises a Discord guildMemberAdd event into a log:subscribe shape
...
/**
 * Normalises a Discord guildMemberRemove event into a log:unsubscribe shape.
...
export function normalizeMessageCreateEvent(
  ...
  return {
    ...
    // Map Discord MentionManager.users Collection to { [userId]: '@username' } contract shape
...
export function normalizeMessageReactionAddEvent(
  ...
  return {
    ...
    // MQTT field required by PROTO_EVENT_MESSAGE_REACTION — Discord has no equivalent
    offlineThreadingID: '',
  };
}

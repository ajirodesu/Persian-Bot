/**
 * Persian — Unified Event Contract
 *
 * Single source of truth for every event shape that crosses the handler layer.
 * Platforms (Discord, Telegram) must normalize their native events into these
 * shapes before passing them to the handler layer.
 *
 * ARCHITECTURE:
...
/**
 * Normalizes any raw platform event into the unified Persian event contract.
 *
 * This function is a structural pass-through that ensures every expected key
 * exists even when the source omits it (null-safety).
 *
 * Platforms must adapt their native event shape to use the correct
...
export function formatEvent(event: Record<string, unknown>): UnifiedEvent {
  ...
  switch (event['type']) {
    ...
    // message_unsend: emitted when a sender retracts a message
    case EventType.MESSAGE_UNSEND:
      return {
        ...
        // Intentional: preserve the undefined sentinel
    ...
    case 'change_thread_image': {
      // change_thread_image is a standalone top-level type;
      ...
      return {
        ...
        logMessageData: {
          // image is null-safe; removed-photo events may carry a null image payload
          image: img
            ? {

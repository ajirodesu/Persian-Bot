...
export interface SessionLifecycle {
   ...
   * @param signal - Optional shutdown signal name, forwarded to the platform's own cleanup.
   * @param persist - When true (default), the stopped state is written to the DB
   *   `isRunning` column so the session-loader skips it on the next boot. Pass
   *   false for a process-wide shutdown (see SessionManager.stopAll) so the bot
   *   resumes automatically when the process restarts.
   */
...
class SessionManager extends EventEmitter {
  ...
  /**
   * Same in-memory/dashboard effect as markInactive, but WITHOUT writing
   * isRunning=false to the DB.
   *
   * Used exclusively by the process-wide graceful shutdown path (stopAll, fired
   * from the SIGINT/SIGTERM handler in app.ts). A server restart is a temporary
   * transport teardown, not a user request to stop the bot — the persisted
   * isRunning flag must stay true so session-loader auto-resumes this session
   * the moment the new process boots. Without this distinction, every running
   * bot would be (incorrectly) marked isRunning=false on every restart and stay
   * offline in the dashboard until someone clicked Start again.
   */
   ...
   * live transports cleanly without flipping isRunning=false in the DB, otherwise
   * every running bot would come back up as "offline" and require a manual Start
   * after each restart. The in-memory active set and dashboard status both still
   * flip to inactive immediately (via markInactiveTransient) so nothing appears
   * to hang; only the persisted "should this bot run" flag is left untouched.
   */

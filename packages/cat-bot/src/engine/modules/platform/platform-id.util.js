 ...
 */
export function toPlatformNumericId(platform) {
    ...
    const id = PLATFORM_TO_ID[platform];
    if (id === undefined) {
        throw new Error(`[platform-id] Unknown platform: "${platform}". ` +
 ...
 */
export function fromPlatformNumericId(id) {
    const name = ID_TO_PLATFORM[id];
    if (name === undefined) {
        throw new Error(`[platform-id] Unknown platform numeric id: ${id}. ` +
            `Ensure it exists in ID_TO_PLATFORM in src/constants/platform.constants.ts.`);
    }
    return name;
}
//# sourceMappingURL=platform-id.util.js.map
...
export class SessionLogger {
    ...
    #format(level, message, extra) {
        const metaObj = extra !== undefined ? { ...extra } : {};
        const metaStr = Object.keys(metaObj).length > 0 ? ` ${JSON.stringify(metaObj)}` : '';
        const colorFn = LEVEL_COLORS[level] ?? ((t) => t);
        ...
        return (chalk.white(getTimestamp()) +
 ...
 * ```
 */
export function createSessionLogger(meta) {
    return new SessionLogger(meta);
}
//# sourceMappingURL=session-logger.lib.js.map
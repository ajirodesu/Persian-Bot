...
export function normalizeTelegramEvent(
  ...
  return {
    ...
    isGroup,
    // Extract Telegram @mention entities into { [userId|username]: mentionText } contract shape
    mentions: (() => {
    ...
    timestamp: msg?.date ? msg.date * 1000 : null, // Telegram date is Unix seconds; multiply for ms
    messageReply,
  };
}
...
/**
 * Normalises a Telegram left_chat_member update into the EventType.EVENT shape
 * with logMessageType 'log:unsubscribe'.
...
export function normalizeTelegramReactionEvent(
  ...
  return {
    ...
    // MQTT field required by PROTO_EVENT_MESSAGE_REACTION — Telegram has no equivalent
    offlineThreadingID: '',
  };
}
...
export function buildTelegramMentionEntities(
  ...
  for (const { tag, user_id } of mentions) {
    let searchFrom = 0;
    // indexOf loop finds every occurrence using fromIndex to advance past each match

 ...
 * errors on names containing hyphens. Normalise here so meta.name values like
 ...
 *
 * When prefix is '/', commands exporting both meta.name and meta.description
...
export async function registerSlashMenu(
  ...
  // Fingerprint the current slash-eligible command meta so we can detect
  ...
  if (prefix === '/') {
    ...
    for (const [key, mod] of commands) {
      ...
      // Warn so module authors know their meta.name or description was mutated before

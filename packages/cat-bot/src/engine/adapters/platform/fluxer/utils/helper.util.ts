/**
 * Fluxer Platform — Shared Utilities
 *
 * General-purpose helpers used by the wrapper layer:
 *   - streamToBuffer: async iteration collector for Fluxer streams
 *   - urlToStream: re-exported from the shared streams module
 *   - buildFluxerMentionMsg: replaces @tag placeholders with <@userId> format
 *
 ...
 * Preferred over EventEmitter (.on 'data') for streams which may be async-iterables.
 */
...
/**
 * Replaces @tag placeholders in a message body with Fluxer's native <@userId> mention format.
 * Fluxer renders <@userId> as a highlighted clickable name and dispatches a notification.
 ...
 */
export function buildFluxerMentionMsg(
  ...
  for (const { tag, user_id } of payload.mentions!) {
    // <@userId> is Fluxer's user mention format — renders as a highlighted, clickable name

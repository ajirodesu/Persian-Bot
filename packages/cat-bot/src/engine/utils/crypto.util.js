...
function getKey() {
    ...
    if (keyHex.length !== KEY_LENGTH * 2) {
        throw new Error(`[crypto] ENCRYPTION_KEY must be exactly 64 hex characters (32 bytes / 256 bits). ` +
            `Got ${keyHex.length} characters.`);
    }
    const key = Buffer.from(keyHex, 'hex');
    // Double-check after decode — invalid hex chars silently produce shorter buffers
    if (key.length !== KEY_LENGTH) {
        throw new Error('[crypto] ENCRYPTION_KEY contains non-hex characters. ' +
...
export function decrypt(value) {
    ...
    if (parts.length !== 3 || !parts[0] || !parts[1] || !parts[2]) {
        throw new Error('[crypto] Malformed encrypted credential value. ' +

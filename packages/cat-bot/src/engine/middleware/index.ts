...
use.onEvent([
  // Fast path: suppresses goodbye.ts when a member was explicitly removed by the
  // `kick` command or `badwords`/`antispam` auto-kicks. These commands pre-register the target
  ...
  enforceCommandKick,
  // Suppresses welcome.ts greeting for warn-banned rejoining members — checkwarn.ts owns the
  // kick notification for the same log:subscribe event; a simultaneous "Welcome!" contradicts it.
  // Also suppresses goodbye.ts farewell for bot-initiated warn-ban kicks on log:unsubscribe —

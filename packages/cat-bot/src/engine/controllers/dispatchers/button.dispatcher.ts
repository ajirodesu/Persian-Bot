 ...
 *   1. Native button clicks (Discord, Telegram) → handleButtonAction()
 *   2. Text-menu number replies (fallback) → dispatchButtonFallback()
 *
 ...
 * on platforms without native interactive button components.
 *
 ...
 * on Discord and Telegram persist until the message is deleted or edited.
 */
export async function dispatchButtonFallback(
  ...
  // Strip ~userId scope suffix before looking up the handler — button fallback
  ...
  // click on Discord or Telegram — no platform branching inside onClick() needed.
 ...
 * Routes to the owning command's button[buttonId].onClick() handler.
 *

...
// ============================================================================
// HELP TOOL UTILITIES
// Mirrors the /help command exactly — kept in sync so the agent sees the same
// filtered, paginated command list as the real command.
// ============================================================================
...
function getCanonicalMods(
  ...
  result.sort((a, b) => {
    const an = String(
      (a['meta'] as Record<string, unknown> | undefined)?.['name'] ?? '',
    );
    const bn = String(
      (b['meta'] as Record<string, unknown> | undefined)?.['name'] ?? '',
    );
...
export const initialize = async (
  ...
  // 3. Hide commands above the user's privilege ceiling. Mirrors /help: a Set
  //    of accessible roles (PREMIUM is a sub-admin tier, so a simple numeric
  //    ceiling would incorrectly hide premium commands from premium users).
  ...
  if (arg && isNaN(Number(arg))) {
    ...
    const modCfg = mod['meta'] as Record<string, unknown> | undefined;
    const canonicalName =
    ...
    const cfg = mod['meta'] as Record<string, unknown>;

/**
 * /short — Google URL Shortener
 *
 * Shortens a URL via the Delirius proxy for Google's shortener service.
 *
 * Flow:
 *   User: /short https://www.delirius.store
 *   Bot:  🔗 Google Shortener
...
async function fetchShortUrl(targetUrl: string): Promise<string | null> {
  const url = createUrl('delirius', '/shorten/googleshort', { url: targetUrl });
  console.log('[short] → GET', url);
  ...
  try {
    ...
    if (status < 200 || status >= 300 || !data || data.status !== true) {
      console.error('[short] ✗ bad response', { status, apiStatus: data?.status });
      return null;
    }

    console.log('[short] ✓ success for', targetUrl);
    return typeof data.data === 'string' ? data.data : null;
  } catch (err) {
    const error = err as AxiosError;
    console.error('[short] ✗ request failed:', error.code ?? error.message);
    return null;
  }
}

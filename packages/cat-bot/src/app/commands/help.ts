 ...
 *   Author  : John Lester
 *
 * Pagination: 20 commands per page, alphabetically sorted by canonical meta.name.
 * Aliases collapse — getCanonicalMods() deduplicates by meta.name before rendering.
 *
 ...
 * Deduplication is keyed on meta.name (the canonical display name) so the
...
export const onCommand = async ({
  ...
  if (arg && isNaN(Number(arg))) {
    ...
    // Build usage display lines — string[] allows multiple usage patterns per command.

 ...
 *   - random-d.uk   → `{ url }`
 *   - Dog CEO       → `{ message }`
 *   - shibe.online  → `["url"]`             (array of plain strings)
 *
 * Commands:
 ...
 *   /bird  — random bird image (aliases: birdpic, birdimage, tweet)
 *
...
const ANIMAL_CONFIGS: AnimalConfig[] = [
  ...
  {
    name: 'bird',
    aliases: ['birdpic', 'birdimage', 'tweet'],
    label: 'Random Bird Image',
    emoji: '🐦',
    buttonLabel: '🔁 Another Bird',
    apiUrl: 'https://shibe.online/api/birds',
    apiParams: { count: '1' },
    extractUrl: (data) => (data as string[] | undefined)?.[0] || null,
  },
];

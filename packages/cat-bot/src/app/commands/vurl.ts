 ...
 * /vurl — vURL Shortener
 *
 * Shortens a URL via the Delirius proxy for vURL.
 *
 * Flow:
 *   User: /vurl https://www.delirius.store
 *   Bot:  🔗 vURL Shortener
 *         🔸 Original: https://www.delirius.store
 *         🔹 Shortened: https://vurl.com/...
 */
...
async function fetchShortUrl(targetUrl: string): Promise<string | null> {
  const url = createUrl('delirius', '/shorten/vurl', { url: targetUrl });
  console.log('[vurl] → GET', url);
  ...
  try {
    ...
    if (status < 200 || status >= 300 || !data || data.status !== true) {
      console.error('[vurl] ✗ bad response', { status, apiStatus: data?.status });
      return null;
    }

    console.log('[vurl] ✓ success for', targetUrl);
    return typeof data.data === 'string' ? data.data : null;
  } catch (err) {
    const error = err as AxiosError;
    console.error('[vurl] ✗ request failed:', error.code ?? error.message);
    return null;
  }
}

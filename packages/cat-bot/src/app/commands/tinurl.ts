/**
 * /tinurl — TinyURL Shortener
 *
 * Shortens a URL via the Delirius proxy for TinyURL.
 *
 * Flow:
 *   User: /tinurl https://www.delirius.store
 *   Bot:  🔗 TinyURL Shortener
 *         🔸 Original: https://www.delirius.store
 *         🔹 Shortened: https://tinyurl.com/...
 */
...
async function fetchShortUrl(targetUrl: string): Promise<string | null> {
  const url = createUrl('delirius', '/shorten/tinyurl', { url: targetUrl });
  console.log('[tinurl] → GET', url);
  ...
  try {
    ...
    if (status < 200 || status >= 300 || !data || data.status !== true) {
      console.error('[tinurl] ✗ bad response', { status, apiStatus: data?.status });
      return null;
    }

    console.log('[tinurl] ✓ success for', targetUrl);
    return typeof data.data === 'string' ? data.data : null;
  } catch (err) {
    const error = err as AxiosError;
    console.error('[tinurl] ✗ request failed:', error.code ?? error.message);
    return null;
  }
}

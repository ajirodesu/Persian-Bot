...
- Prefer doing the smallest amount of work that satisfies the request. When several independent commands are requested, put them together in one `test_command` call so they can be processed as one turn and delivered in one message. Do not serialize independent work unnecessarily.
...
1. `test_command` with one `commands` array containing every command needed for this turn. Preserve order when one depends on another. Independent tool calls may be emitted together and are executed concurrently by the agent. It returns `calls` plus `attachment_key` / `binary_attachment_key` / `button_key`.
2. `send_result` exactly once with a short, original AI-generated introduction and the non-null keys. For media, do not replace or rewrite the command's original caption: the system places that caption below your introduction with a blank line. A suitable style is: "Here's your random meme, generated by AI dynamically."
